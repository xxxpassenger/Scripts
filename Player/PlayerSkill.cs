...
public class PlayerSkill : MonoBehaviour
{
    ...
    //Private Variable
    private PlayerController controller;  //人物控制组件引用
    private PlayerStatus playerStatus;   //人物状态组件引用
    //private BottomBar botBar;
    //private AnimationManager animationManager;
    private bool setupSkillTimer;       //技能发动所需时间
    [HideInInspector]
    public bool oneShotResetTarget;         //一击后恢复


    [HideInInspector]
    public bool canCast;        //可消耗 （魔法）


    private string typeofSkill;         //技能类型
    private int currentUseSkill;        //当前使用中的技能数组下标
    private GameObject magicCircle;     //魔法圈


    //Editor Variable
    [HideInInspector]
    public int sizePassiveSkill = 0;        //被动技能数量
    [HideInInspector]
    public int sizeActiveAttack = 0;        //主动技能数量（攻击型）
    [HideInInspector]
    public int sizeActiveSupport = 0;       //主动技能数量（辅助型）
    [HideInInspector]
    public List<bool> showPassiveSize = new List<bool>();       //显示被动技能数量
    [HideInInspector]
    public List<bool> showActiveAttackSize = new List<bool>();      //显示主动技能数量（攻击型）
    [HideInInspector]
    public List<bool> showActiveSupportSize = new List<bool>();     //显示主动技能数量（辅助型）
    ...
    public void ActiveSkill(string skillType, int indexSkill)
    {
        ...
        {
            if (activeSkillAttack[indexSkill].skillType == SkillType.Instance)   //非指向性AOE攻击技能（角色周围）
            {
            ...
            else if (activeSkillAttack[indexSkill].skillType == SkillType.LockTarget)   //指向性技能
            {
                if (activeSkillAttack[indexSkill].skillFX != null)
                    Instantiate(activeSkillAttack[indexSkill].skillFX, transform.position, Quaternion.identity);
                ...
                if (activeSkillAttack[indexSkill].targetSkill == TargetSkill.MultipleTarget)  //目标为多个
                {
                    GameObject loadSkillArea = (GameObject)Resources.Load("AreaSkill");
    ...
    //Count duration buff  计算buff技能持续时间是否结束
    ...
    //Text damage
    public void InitTextDamage(Color colorText, string damageGet) //显示伤害数值
    {
        // Init text damage
