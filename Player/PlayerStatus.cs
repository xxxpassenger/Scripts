...
public class PlayerStatus : MonoBehaviour
{
    ...
    {
        public int lv, hp, mp, atk, def, spd, hit;  //等级，血量，魔量，攻击力，护甲，速度，命中
        public float criticalRate, atkSpd, atkRange, movespd, exp;  //暴击率，攻击速度，攻击范围，移动速度，经验值
    }
    ...
    [System.Serializable]
    public class StatusGrowth  //成长增益
    {
        public int hp, mp, atk, def, spd, hit;
        public float criticalRate, atkSpd, atkRange, movespd;
    }
    ...
    public int maxLv = 99; // Max level  最高等级
    public int pointPerLv; // When you level up you gain x point per level 每升一级需要的点数
    public int startExp; // start level 1 exp 初始经验
    public float multipleExp; //multiple exp per level  

    [HideInInspector]
    public int hpMax, mpMax, hpCurrent, mpCurrent;  //最大血量，最大魔量，当前血量，当前魔量

    [HideInInspector]
    public float expMax;  //最大经验

    public Attribute status;  //main status  主属性
    public SubAttribute statusAdd; //add status(from equip,skill)   增益属性（来自装备,技能）
    public SumAttribute statusCal; //summary status 状态计算
    public SumEquip statusEquip;        //设备属性
    public StatusGrowth statusGrowth, growthSetting; //status growth when level up  成长属性，自定义设置

    public float hpRegenTime; //hp regen per second     秒回血量
    public float mpRegenTime; //mp regen per second     秒回魔量
    ...
    void Update()
    {
        ...
        if (statusCal.hp <= 0)      //挂掉的角色不能靠秒回血量魔量复活
        {
